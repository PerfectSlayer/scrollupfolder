...
fr.hardcoding.scrollupfolder = {
	...
	urlbar: {
		...
		onKeyDown: function(event) {
			...
			if (url !== null) {
				// Get selected index
				var selectedIndex = UrlPanel.getSelectedIndex(window);
				if (selectedIndex !== -1) {
	...
	processPaths: function(tab) {
		...
		// Get URL in urlbar
		var urlbarUrl = Urlbar.getUrl(tab.window);
		// Check urlbar URL
		if (urlbarUrl !== null) {
			// Check if urlbar URL maches one of path
			var urlbarIndex = tab.SUFPaths.indexOf(urlbarUrl);
			if (urlbarIndex !== -1) {
				...
				var urlbarPaths = fr.hardcoding.scrollupfolder.computePaths(urlbarUrl);
				// Check each computed paths from urlbar URL
				for (var urlbarPathsIndex = 1, urlbarPathsCount = urlbarPaths.length; urlbarPathsIndex<urlbarPathsCount; urlbarPathsIndex++) {
