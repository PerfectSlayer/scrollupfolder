...
fr.hardcoding.scrollupfolder = {
	...
	urlpanel: {
		...
		onClick: function(event) {
			...
			if (SimplePrefs.prefs.controlMode == 1) {
				// Load url in current tab
				fr.hardcoding.scrollupfolder.loadUrl(item.label);
			} else {
	...
	button: {
		 ...
		 */
		init: function() {
			// Add clicking event on url button
	...
	loadUrl: function(url, event) {
		try {
			...
			var cleanedUrl = Url.URL(uri);
			// Load valid URL
			fr.hardcoding.scrollupfolder.loadValidUrl(cleanedUrl.spec, event);
		}
