...
// Declare urlbar-container listeners
var scrollListeners = [];
...
function initialize (browserWindow) {
	...
	// Get urlbar element
	var urlbar = document.getElementById('urlbar');
	// Add scrolling event listener on urlbar-container
	urlbar_container.addEventListener('DOMMouseScroll', function(event) {
		notifyListeners(scrollListeners, event)
	}, true);
	// Add event listener for each type of event on urlbar
	for (var type in listeners) {
		// Create temporay function to fix the scope
		function fixScope(eventType) {
			// Add event listener
			urlbar.addEventListener(eventType, function (event) {
				// Notify listeners
				notifyListeners(listeners[eventType], event);
			}, true);
		};
